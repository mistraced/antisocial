...
#define STATIC_HOOK_CODEPAGE_SIZE 0x8000
#define STATIC_HOOK_DATAPAGE_SIZE 0x8000
...
NSMutableData* add_hook_section( NSMutableData* macho )
{
    ...
    for ( int i = 0; i < header->ncmds; i++ )
    {
        ...
        if ( lc->cmd == LC_SEGMENT_64 )
        {
            ...
            {
                NSLog( @"section[%d] = %s/%s offset=%x vm=%p:%p", j, sec[ j ].segname, sec[ j ].sectname,
...
NSString* StaticInlineHookPatch( char* machoPath, uint64_t vaddr, char* patch, void* replaceTarget )
{
    ...
    if ( newPath )
    {
        ...
        if ( !macho )
            return [NSString stringWithFormat:@"?????(can't find file):\n Documents/static-inline-hook/%s", machoPath];
    } else
    {
        macho = load_macho_data( path );
        if ( !macho )
            return [NSString stringWithFormat:@"??????(can't read file):\n.app/%s", machoPath];
    }
    ...
    for ( int i = 0; i < STATIC_HOOK_CODEPAGE_SIZE / sizeof( StaticInlineHookBlock ); i++ )
    {
        ...
        if ( hookBlock[ i ].hook_vaddr == 0 )
        {
            ...
            NSLog( @"found empty StaticInlineHookBlock %d %p=>%p\n", i, targetRVA, targetData );
    ...
    hookBlock->target_replace = replaceTarget;
